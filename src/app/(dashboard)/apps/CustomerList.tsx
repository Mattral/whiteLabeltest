...
const CustomerList = () => {
      ...
      },
      {
        ...
        Cell: ({ row }: { row: Row }) => (
          <Stack direction="row" spacing={0}>
            <Tooltip title="View">
              <IconButton
                color={mode === ThemeMode.DARK ? 'secondary' : 'primary'}
  ...
  return (
    <MainCard title="Customer List">
      <ScrollX>
        <ReactTable columns={columns} data={data} modalToggler={() => setCustomerModal(true)} renderRowSubComponent={({ row }: { row: Row }) => <CustomerView data={row.original} />} />
      </ScrollX>
      {/* Add & Edit customer dialog */}
      <CustomerModal open={customerModal} modalToggler={() => setCustomerModal(false)} customer={customer} />
      {/* Customer Delete */}
